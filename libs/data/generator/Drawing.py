...
def drawing_panels(root):
    ...
    interpret, mode, part, operator_list, int_list_1, int_list_2, int_list_3, pos_list, final_expression = math_parser(math_conditions,
                                                                                                     geom_conditions,
                                                                                                     prob_type,
                                                                                                     positions,
                                                                                                     show_center)
    int_list_1 = [int(x) for x in int_list_1]
    int_list_2 = [int(x) for x in int_list_2]
    int_list_3 = [int(x) for x in int_list_3]
    img1, img2, img3 = copy.deepcopy(img), copy.deepcopy(img), copy.deepcopy(img)
    answer_1, img_1, mark_1, bboxes_1 = disp_num(img1, int_list_1, pos_list, False, font_size, show_center)
    answer_2, img_2, mark_2, bboxes_2 = disp_num(img2, int_list_2, pos_list, False, font_size, show_center)
    answer_3, img_3, mark_3, bboxes_3 = disp_num(img3, int_list_3, pos_list, True, font_size, show_center)
    prob_answer = answer_3
    prob_operator = operator_list
    prob_images = [img_1, img_2, img_3]
    return prob_answer, prob_operator, int_list_1, int_list_2, int_list_3, show_center, interpret, mode, part, mark_3, prob_images, final_expression, bboxes_3
...
def draw_partition(geom_conditions):
    ...
    if geom_type == "square":
        position = partition_square(img, CENTER[0], CENTER[1], L2, geom_part, DEFAULT_WIDTH)
    elif geom_type == "circle":
        position = partition_circle(img, CENTER[0], CENTER[1], L, geom_part, DEFAULT_WIDTH)
    elif geom_type == "hexagon":
        position = partition_hexagon(img, CENTER[0], CENTER[1], L, geom_part, DEFAULT_WIDTH)
    return img, position
...
def draw_triangle(img, center, l, inverse, thickness):
...
def draw_rectangle(img, center, l, shape, thickness):
...
def draw_circle(img, center, l, thickness):
    x, y = center[0], center[1]
    center = (int(x), int(y))
...
def draw_hexagon(img, center, l, thickness):
    x, y = center[0], center[1]
